...
static void vTaskWiFiTest(void *pvParameters);
...
volatile int is_interrupt_called = 0;
...
int main(void)
{
  ...
  /* 4. Reset the ESP32 */
  LOG_INFO("Holding ESP32 in Reset...\r\n");
  HAL_GPIO_WritePin(ESP_EN_GPIO_Port, ESP_EN_Pin, GPIO_PIN_RESET);
  HAL_Delay(50);

  /* 5. Release Reset -> ESP32 Boots */
  LOG_INFO("Releasing Reset (ESP32 Booting into Firmware)...\r\n");
  HAL_GPIO_WritePin(ESP_EN_GPIO_Port, ESP_EN_Pin, GPIO_PIN_SET);

  /* 6. STRAPPING WINDOW (D0 MUST BE LOW) */
  /* Wait just enough for ESP32 to check GPIO 2 (approx 50-100ms) */
  HAL_Delay(100);

  /* 7. NOW Initialize SDMMC */
  /* This enables the STM32 Internal Pull-up on D0, which is needed for Data Transfer */
  LOG_INFO("Strapping done. Initializing SDMMC (D0 Pull-up ON)...\r\n");
  MX_SDMMC1_Init();

  /* 8. WAIT FOR FIRMWARE */
  /* Now that the Bus is valid (High), give ESP32 time to boot OS & queue packet */
  LOG_INFO("Waiting 2s for ESP32 App to load on valid bus...\r\n");
  HAL_Delay(2000);
  ...
  xTaskCreate(vTaskWiFiTest, "ESP Handshake Task", 1024, NULL, 3, NULL);
...
static void vTaskApplicationMain(void *pvParameters)
{
  ...
  while (1)
  {
    if (counter == 20)
    {
      LOG_INFO("Main Task Heartbeat: %d\r\n", counter / 20);
    }
    counter++;
...
static void vTaskWiFiTest(void *pvParameters)
{
  /* Allow ESP32 a moment to boot its firmware */
  vTaskDelay(pdMS_TO_TICKS(2000));

  LOG_INFO("Initializing ESP-Hosted Driver...\r\n");

  /* * NEW INITIALIZATION FLOW
   * In newer ESP-Hosted versions, calling esp_wifi_init() triggers the 
   * underlying transport initialization automatically via the port layer.
   */

  /* 1. Initialize Wi-Fi (Standard ESP-IDF API style) */
  wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
  
  /* This function internally calls the transport init (SDIO) */
  if (esp_wifi_init(&cfg) == ESP_OK) 
  {
      LOG_INFO("✅ Wi-Fi Driver Started!\r\n");
      
      /* 2. Start Wi-Fi */
      esp_wifi_set_mode(WIFI_MODE_STA);
      esp_wifi_start();

      /* 3. Get MAC Address (Proof of Life) */
      uint8_t mac[6] = {0};
  ...
  {
      LOG_ERROR("❌ esp_wifi_init failed. Check Transport/Resistors.\r\n");
...
static void MX_SDMMC1_Init(void)
{
  ...
  /* Configure D1-D3 and CLK pins (Port C) */
  GPIO_InitStruct.Pin = SDMMC1_D1_Pin | SDMMC1_D2_Pin | SDMMC1_D3_Pin | SDMMC1_CLK_Pin;
  HAL_GPIO_Init(SDMMC1_Data_GPIO_Port, &GPIO_InitStruct);

  /* Configure D0 pin (Port B) */
  GPIO_InitStruct.Pin = SDMMC1_D0_Pin;
  GPIO_InitStruct.Pull = GPIO_PULLUP; // Enable pull-up on D0
  HAL_GPIO_Init(SDMMC1_Data_GPIO_Port, &GPIO_InitStruct);
   ...
   */
  SDMMC1->CLKCR = (120U << SDMMC_CLKCR_CLKDIV_Pos); // ~1.6 MHz
